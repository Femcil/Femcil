(function () {
  ...
  (function () {
    ...
    const textColoration = (ctx, str, x, y, maxWidth, operation) => {
      ...
      let sections = str.split("§").map((s) => {
        let color = colors[parseInt(s.charAt(0), 36)];
        return {
          color: color,
      ...
      let offset = 0;
      if (align == "right" || align == "end") offset = 1;
      else if (align == "center") offset = 1 / 2;
      if (offset) {
  ...
  var wsUrl = null,
    ...
    EMPTY_NAME = "im a dumb bitch with no name",
  ...
  function wsMessage(data) {
    ...
    switch (packetId) {
      ...
      case 0x63: // chat message
        ...
        if (server && name !== "§6ƧΣЯVΣЯ") name = "§1F§2Σ§3M§4ᄃ§5I§6ᄂ";
        if (admin) name = "§4ΛDMIП" + name;
        if (mod) name = "§2MӨDƧ" + name;
  ...
  function buildGallery() {
    ...
    for (var i = 0; i < sortedKeys.length; i++) {
      ...
      c +=
        '<img class="circular" src="http://mkskinsserver.glitch.me/' +
        name +
        '.png">';
    ...
    for (var i = 0, len = latestMessages.length; i < len; i++)
      lines.push([
        ...
        },
        {
          ...
          color: settings.darkTheme ? "#C9C9C9" : "#ADADAD",
        },
      ]);
    var width = 0;
    var height = 30 * len + 2;
  ...
  function drawGrid() {
    mainCtx.save();
    mainCtx.lineWidth = 1;
    mainCtx.strokeStyle = settings.darkTheme ? "#000" : "#000";
  ...
  function drawBackgroundSectors() {
    ...
    var sectorCount = 5;
    var sectorNames = ["ĄβÇĐΞ", "12345"];
    var w = border.width / sectorCount;
    var h = border.height / sectorCount;

    toCamera(mainCtx);
    mainCtx.fillStyle = settings.darkTheme ? "#00fff6" : "#FF0000";
  ...
  function drawMinimap() {
    ...
    mainCtx.fillStyle = "#fa0000";
    mainCtx.globalAlpha = 0.4;
    mainCtx.fillRect(beginX, beginY, width, height);
    mainCtx.globalAlpha = 1;

    var sectorCount = 5;
    var sectorNames = ["αႦƈԃҽ", "12345"];
    ...
    mainCtx.fillStyle = settings.darkTheme ? "#1dfdea" : "#DDD";
    mainCtx.textBaseline = "middle";
    mainCtx.textAlign = "center";
  ...
  function drawGame() {
    ...
    var height = 2;
    mainCtx.fillStyle = settings.darkTheme ? "#3dff5d" : "#A5A5A5";
    mainCtx.textBaseline = "top";
    if (!isNaN(stats.score)) {
      mainCtx.font = "30px Ubuntu";
      mainCtx.fillText("𝙎ᴄᴏʀᴇ: " + stats.score, 2, height);
      height += 30;
    }
    mainCtx.font = "20px Ubuntu";
    var gameStatsText = ~~stats.fps + " FPS";
    if (!isNaN(stats.latency)) gameStatsText += " " + stats.latency + "ᴍꜱ ᴘɪɴɢ";
    ...
    if (settings.showChat && (chat.visible || isTyping)) {
      mainCtx.globalAlpha = isTyping
        ? 1
        : Math.max(10000 - syncAppStamp + chat.waitUntil, 0) / 10000;
      mainCtx.drawImage(
        chat.canvas,
        10 / camera.viewportScale,
        (mainCanvas.height - 30) / camera.viewportScale - chat.canvas.height
      );
      mainCtx.globalAlpha = 1;
    }
    drawMinimap();
    ...
    if (minionControlled) {
      ...
      var text = "You Are Controling A Bot";
